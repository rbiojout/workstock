class SlotsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @slot.save
        # automatically add the slot to all WorkDays
        WorkDay.all.each do |wd|
          WorkGrid.create(:work_day_id => wd.id, :slot_id => @slot.id)
        end
  ...
  private
    ...
    def slot_params
      params.require(:slot).permit(:start_time, :duration, :prefered)
    end
end
